...
export default function Header({ showToggle, setShowToggle }: any) {
  const [checked, setChecked] = useState<boolean>(false);

  useEffect(() => {
    if (typeof window !== "undefined") {
      setChecked(window.localStorage.getItem("theme") == "dark" ? true : false);
  ...
  return (
    <>
      ...
      >
        ...
        {showToggle && (
          <div
            className={`react-toggle ${checked && "react-toggle--checked"}`}
            onClick={() => {
              setChecked(!checked);
              if (typeof window !== "undefined") {
                if (window.localStorage.getItem("theme") == "dark") {
